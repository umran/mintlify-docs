---
title: "Obligation Report Format: JSON Structure and Fields"
sidebarTitle: "Obligations Report"
description: "Conseqa writes a JSON obligation report with every requirement's verdict, scope (l0_only or runtime_dependent), evidence, and assumptions. The exact shape is what conseqa --report emits."
---
...
The exact keys and structure match what the current engine emits. The description below reflects the report format as read by `conseqa-viz` (format 4), which adds proof `scope` and `remedy` fields to earlier shapes. If you are consuming this output programmatically, pin against the `format` field at the top level.

## Report Structure

The top-level report object contains the format version, the model revision the report was produced against, the list of obligations, and any model-wide notes.
...
<ParamField path="format" type="integer" required>
  The report format version. `conseqa-viz` expects format 4 for reports produced by the current engine and warns when versions do not match.
</ParamField>
...
<ParamField path="scope" type="string">
  Present for `proven` obligations. `l0_only` if no L1 fact was required; `runtime_dependent` if at least one L1 fact was necessary. A `runtime_dependent` proof holds of the declared runtime realization and must be re-examined when that realization changes.
</ParamField>

<ParamField path="remedy" type="string">
  Present for unproven (`unknown`) serialization and ordering obligations. `application` if the missing facts belong to L0; `runtime` if they belong to L1. This is a routing hint, not a promise that declaring one fact closes the argument.
</ParamField>
...
<Note>
  The `assumptions` list for a proven obligation names every declared fact the proof depends on. Review these after any implementation change to confirm the real system still provides those semantics, and check the `scope` field to know whether the proof depends on runtime topology.
</Note>
...
When a report is loaded, `conseqa-viz` colors operations, topics, and state machines by their worst obligation status, adds verdict chips to requirement rows in the operation view, and populates the obligations panel with expandable cards showing assumptions, evidence, and counterexample traces. The obligations panel includes a layer filter (`any / L0 / L1`) that uses `scope` for proven obligations and `remedy` for unproven ones.
