...
**10 proven obligations** — including serialization, ordering, and recoverability for `apply_payment` and `create_order`, and result replay for `create_order`. These operations use keyed commits, identified events, and subscription dispatch with `key: grouping_key`, `member_assignment: consistent_hash`, and `member_concurrency: bounded(1)` on the target pool.
