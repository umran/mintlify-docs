...
The inline transaction is `deduplicated_by` the request's `upload_id`, which is also the idempotency key. The `establish_effect_intent` step declares the publication contract inline, captures its concrete payload from `values`, and binds the intent, all atomically with the commit. This means:

- A second attempt under the same `upload_id` resolves the prior commit and restores the `intent.complete_upload.publish_uploaded` binding without re-inserting the video record.
...
`transcode_video` is the most interesting case. It calls an external transcoding engine and branches on the result. The engine's contract is declared inline at the `execute_effect` site:
...
Transaction-read bindings are local to the transaction execution that produced them and are always treated as unknown for replay purposes.
