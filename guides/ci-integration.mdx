...
## Obligation scope in the report

Each obligation in the report now carries a `scope` field:

- `l0_only`: the proof required no runtime facts.
- `runtime_dependent`: the proof consumed at least one L1 fact (delivery semantics, routing, pool concurrency, outbox runtime, and so on).

This is useful for CI matrices that verify both a minimal L0-only model and a full model with runtime topology. Removing the `runtime` block from a valid model leaves it valid: runtime-dependent proofs simply become `unproven`, never structural errors. A CI job that strips the runtime block and re-runs the verifier can confirm that the L0 layer stands on its own, while the full-runtime variant checks the complete architecture.
...
jobs:
  verify-l0:
    steps:
      - uses: actions/checkout@v4
      - run: yq 'del(.runtime)' model.yaml > model.l0.yaml
      - run: conseqa model.l0.yaml --report report.l0.json
      - run: jq -e '[.obligations[] | select(.scope == "l0_only" and .status != "proven")] | length == 0' report.l0.json

  verify-full:
    steps:
      - uses: actions/checkout@v4
      - run: conseqa model.yaml --report report.full.json
      - run: jq -e '[.obligations[] | select(.status != "proven")] | length == 0' report.full.json
```
