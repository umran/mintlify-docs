...
References a field in the payload of a declared `PublicationEffect` or `RequestEffect`. The ID resolves to the inline effect site's `effect_id` (on `execute_effect` or `establish_effect_intent`), or to a state-machine transition side-effect declaration. Use this in `idempotency_key_propagation` to name the target fields in the outbound payload.
...
References a field of a transaction-output binding: a typed value a transaction deliberately exported into the operation's control via `establish_transaction_output` (which supplies the binding's `bind` name, its `schema`, and its `values` derivation at the production site).
...
References a field observed by a `read` step earlier in the **same transaction execution**. The read step's `bind` field names the binding. This source kind is restricted to the transaction that produced the read.
...
The ID names the result binding declared as `bind` at the `execute_effect` or `execute_effect_intent` step. The path resolves against the effect contract's `ok` schema. `effect_result_ok` is an operation-local, attempt-local observation, not a transaction artifact, and does not survive the join after the `match_result`.
...
| `effect` | Payload field of a publication or request effect | Inline effect sites within the admitting operation (`effect_id`) or transition-owned side-effect declarations |
| `transaction_output` | Field of a transaction-output binding | Definitely available at the reference site (forward analysis) |
| `state_machine_subject` | Field on a persistent state machine object | None — unrestricted |
| `transaction_read` | Field observed by a read step in the same transaction | The transaction that produced the read binding |
...
- `transition` effect intents (`effect_intents.<effect_id>.values` per side effect)
...
4. **Recovered artifacts** — transaction-output and effect-intent bindings established by a `deduplicated_by` transaction whose key components are all stable are stable (route B recovery).
5. **Reconstructed artifacts** — transaction-output and effect-intent bindings established by a naturally replayable transaction, with a replay-deterministic derivation, are stable (route A reconstruction).
...
**`effect_result_ok` inside a transition's `effect_intents` map (video streaming):**
